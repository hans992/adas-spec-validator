...
import { FileText, FolderOpen, GitCompareArrows, KeyRound, LogIn, LogOut, Save, UserPlus } from "lucide-react";
...
export function ProjectWorkspace({ model, requirements, modelName, onOpen }: {
  ...
  return (<>
    {reportSnapshot && <ValidationReport projectName={projects.find((project) => project.id === projectId)?.name ?? "Untitled project"} snapshot={reportSnapshot} onClose={() => setReportSnapshot(null)} />}
    <Card className="shadow-md">
