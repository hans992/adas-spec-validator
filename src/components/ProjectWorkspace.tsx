...
export function ProjectWorkspace({ model, requirements, modelName, onOpen }: {
  ...
  return (<>
    {reportSnapshot && <ValidationReport projectName={projects.find((project) => project.id === projectId)?.name ?? "Untitled project"} snapshot={reportSnapshot} reviews={reportReviews} onSaveReview={async (decision) => { const payload = await api(`/api/projects/${projectId}/validations/${reportSnapshot.id}/reviews`, { method: "PUT", body: JSON.stringify(decision) }); setReportReviews((current) => [payload.review, ...current.filter((review) => review.requirement_id !== payload.review.requirement_id)]); }} onClose={() => setReportSnapshot(null)} />}
    <Card className="shadow-md">
